...
-- HUD gọn (corner) local hud = Instance.new("Frame") hud.Name = "HUD" hud.Parent = gui hud.AnchorPoint = Vector2.new(1, 1) hud.Position = UDim2.new(1, -12, 1, -12) hud.Size = UDim2.new(0, 270, 0, 60) hud.BackgroundColor3 = Color3.fromRGB(15, 15, 20) hud.BackgroundTransparency = 0.05 hud.BorderSizePixel = 0 hud.AutomaticSize = Enum.AutomaticSize.Y hud.ClipsDescendants = true
...
local info1 = mkLabel(hud, "FPS: …   Ping: …   Mem: …", 14, false) local info2 = mkLabel(hud, "Players: …   Pos: …", 14, false)
...
-- Menu chi tiết local menu = Instance.new("Frame") menu.Name = "Menu" menu.Parent = gui menu.AnchorPoint = Vector2.new(0, 0) menu.Position = UDim2.new(0, 20, 0, 80) menu.Size = UDim2.new(0, 420, 0, 300) menu.BackgroundColor3 = Color3.fromRGB(12,12,16) menu.BackgroundTransparency = 0.03 menu.BorderSizePixel = 0 menu.Visible = false
...
local menuTitle = mkLabel(header, "⚙️ Kai Performance Monitor", 18, true) menuTitle.Position = UDim2.new(0, 12, 0, 10) menuTitle.Size = UDim2.new(1, -90, 1, -10)

local closeBtn = Instance.new("TextButton") closeBtn.Parent = header closeBtn.AnchorPoint = Vector2.new(1,0) closeBtn.Position = UDim2.new(1, -10, 0, 6) closeBtn.Size = UDim2.new(0, 32, 0, 30) closeBtn.BackgroundColor3 = Color3.fromRGB(30,30,38) closeBtn.Text = "✖" closeBtn.TextColor3 = Color3.fromRGB(240,240,255) closeBtn.Font = Enum.Font.GothamBold closeBtn.TextSize = 14 local cbCorner = Instance.new("UICorner", closeBtn) cbCorner.CornerRadius = UDim.new(0, 8) closeBtn.MouseButton1Click:Connect(function() menu.Visible = false end)
...
-- Nội dung menu local body = Instance.new("Frame") body.Parent = menu body.Position = UDim2.new(0, 12, 0, 46) body.Size = UDim2.new(1, -24, 1, -58) body.BackgroundTransparency = 1
...
local function mkCard(labelText) local card = Instance.new("Frame") card.BackgroundColor3 = Color3.fromRGB(20,20,28) card.BackgroundTransparency = 0.08 card.BorderSizePixel = 0 local c = Instance.new("UICorner", card); c.CornerRadius = UDim.new(0, 14) local s = Instance.new("UIStroke", card); s.Thickness = 1.5; s.Transparency = 0.25 local lab = mkLabel(card, labelText, 12, true); lab.TextTransparency = 0.15; lab.Position = UDim2.new(0, 10, 0, 6) local val = mkLabel(card, "…", 18, true); val.Position = UDim2.new(0, 10, 0, 26) return card, val, s end
...
-- Footer hint local footer = mkLabel(menu, "RightShift: mở/đóng menu • F8: ẩn/hiện HUD • F9: Speed X50", 12, false) footer.Position = UDim2.new(0, 12, 1, -18) footer.TextTransparency = 0.25
